...
class Game:
    ...
    def evaluateMoves(self, moves_list):
        ...
        if len(tmp) <= 1:
            return tmp.copy()
        for move in tmp:
            priority = (move[0] + self.
                        differentiateHeuristic([move[1], move[2],
                                                move[3], move[4]]) + move[0])
            move[0] = priority
        maxPriority = -1000
...
#хм
    ...
    def iterationRollout(self, depth, maxDepth):
        ...
        if depth <= 0:
            self.addRolloutMove(maxDepth)
            self.rollout_moves_lists[-1].append(evaluateGame(self.rollout_moves_lists[-1]))  # we append the value of the moves_list
        return
    ...
    def cardFacedUp(self, card):
        return any((card[0], card[1], 1) in self.game[i] for i in
                   range(len(self.game)))
    ...
    def moveIsLegal(self, move):
        ...
        if 6 <= move[3] <= 12:
            if len(self.game[move[3]]) == 0:
                ...
                return (self.game[move[1]][move[2]][0] ==
                        self.game[move[3]][move[4]][0] - 1 and
                        cardIsRed(self.game[move[1]][move[2]][1]) !=
                        cardIsRed(self.game[move[3]][move[4]][1]))
        if 2 <= move[3] <= 5:
        ...
        if 2 <= move[1] <= 13 and 2 <= move[3] <= 13 and len(self.moves_history) > 9:
            for i in range(1, 8):
                ...
                if (
                    ...
                    if (prevGame[prevMove[1]][prevMove[2]]
